...
// // ===== Activation with parms ====
// struct MyTiledAggCaptureState : public AutoGradCaptureState {
//   bool requires_grad; // 输入x是否需要梯度
//   //是否需要写w，b
// };

// class MyTiledAgg : public OpExprGradFunction<MyTiledAggCaptureState> {
//  public:
//   Maybe<void> Init(const OpExpr& op) override {
//     const auto* fw_op_expr = dynamic_cast<const UserOpExpr*>(&op);
//     CHECK_NOTNULL_OR_RETURN(fw_op_expr);  // NOLINT(maybe-need-error-msg)
...
//   Maybe<void> Capture(MyTiledAggCaptureState* ctx, const TensorTuple& inputs,
//                       const TensorTuple& outputs, const AttrMap& attrs) const override {
//     CHECK_EQ_OR_RETURN(inputs.size(), 1);  // NOLINT(maybe-need-error-msg)
//     ctx->requires_grad = inputs.at(0)->requires_grad();
//     if (!ctx->requires_grad) { return Maybe<void>::Ok(); }
...
//     if (ctx->requires_grad) {
//       const auto& x = ctx->SavedTensors().at(0);
//       in_grads->at(0) = JUST(functional::MyTiledAggregateGrad(x, out_grads.at(0)));
//     }
//     return Maybe<void>::Ok();
//   }
...
// REGISTER_OP_EXPR_GRAD_FUNCTION("my_tiled_aggregate", MyTiledAggregate); 
