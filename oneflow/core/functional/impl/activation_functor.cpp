...
//实现tiled_aggregate_op对应functor并注册
class MyTiledAggregateFunctor {
 public:
  MyTiledAggregateFunctor() {
    op_ = CHECK_JUST(one::OpBuilder("my_tiled_aggregate").Input("x").Input("w").Input("b").Output("y").Build());
  }
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x,
                           const std::shared_ptr<one::Tensor>& w,
                           const std::shared_ptr<one::Tensor>& b) const {
    return OpInterpUtil::Dispatch<one::Tensor>(*op_, {x,w,b});
  }
...
class MyTiledAggregateGradFunctor {
 public:
  MyTiledAggregateGradFunctor() {
    op_ = CHECK_JUST(
        one::OpBuilder("my_tiled_aggregate_grad").Input("x").Input("dy").Output("dx").Build());
  }
  Maybe<Tensor> operator()(const std::shared_ptr<one::Tensor>& x,
...
ONEFLOW_FUNCTION_LIBRARY(m) {
  ...
  m.add_functor<impl::MyTiledAggregateFunctor>("MyTiledAggregate");
  m.add_functor<impl::MyTiledAggregateGradFunctor>("MyTiledAggregateGrad");
